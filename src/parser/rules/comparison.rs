...
use crate::parser::rules::bitwise_term::BitwiseTermRule;
...
impl ParseRule<Box<dyn ASTNode>> for ComparisonRule {
    ...
    fn parse(&self, parser: &mut ExprParser) -> Option<Box<dyn ASTNode>> {
        ...
        let mut expr = parser.apply_rule(BitwiseTermRule {}, "bitwise term expression", None)?;
        ...
        while let Some(operator) = parser.try_consume_match(&matches) {
            ...
            let term = parser.apply_rule(BitwiseTermRule {}, "bitwise term expression", None)?;            
